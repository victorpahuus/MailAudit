#!/bin/bash
# mailaudit.sh — Mail domain/MX security scanner (macOS/Bash 3 compatible)
...
EXTRA_ENDPOINTS=()   # e.g. "host port type"
HOST_ARGS=()
SINGLE_HOST=""
...
Options:
  ...
  --include-cloud     Include Microsoft 365 / Google / hosted MX in host-level tests
...
readarray_compat() {
  local __n="$1"; shift
  local __a=() __l
  while IFS= read -r __l; do [ -n "$__l" ] && __a+=("$__l"); done < <("$@")
  eval "$__n=(\"\${__a[@]}\")"
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --mx) DO_MX=true; shift;;
    -S|--services) SERVICE_MAP="$2"; shift 2;;
    ...
    --smtp-host) h="$2"; shift 2; EXTRA_HOSTS+=("${h}%25:smtp,587:smtp,465:smtps");;
    --imap-host) h="$2"; shift 2; EXTRA_HOSTS+=("${h}%993:imaps");;
    --endpoint)
      ep="$2"; shift 2
...
has_a_record(){ dig +short A "$1" | grep -qE '^[0-9]+' && return 0; dig +short AAAA "$1" | grep -q ':' && return 0; return 1; }
...
run_tests() {
  ...
  # Explicit endpoints tied to this host
  if [[ ${#EXTRA_ENDPOINTS[@]} -gt 0 ]]; then
    for line in "${EXTRA_ENDPOINTS[@]}"; do
      set -- $line
      local eh="$1" ep="$2" et="$3"
      if [[ "$eh" == "$h" ]]; then
        echo
        local uc_et; uc_et="$(printf '%s' "$et" | tr '[:lower:]' '[:upper:]')"
        echo -e "  ${BOLD}Port $ep${NC} ${DIM}(${uc_et})${NC}"
...
if [[ -n "${SINGLE_HOST:-}" ]]; then
  ...
  # Add explicit per-host service maps and endpoints
  if [[ ${#EXTRA_HOSTS[@]} -gt 0 ]]; then
    for x in "${EXTRA_HOSTS[@]}"; do HOSTS+=("$x"); done
  fi
fi

# Dedup hosts (bevar evt. %maps)
if [[ ${#HOSTS[@]} -gt 0 ]]; then
  HOSTS=($(printf "%s\n" "${HOSTS[@]}" \
    | sed 's/\.$//' \
    | awk 'NF' \
...
if $have_hosts; then
  if [ "$PARALLEL" = true ] && command -v parallel >/dev/null 2>&1; then
    # Silence citation notice once (best-effort)
    parallel --citation >/dev/null 2>&1 || true
    for h in "${HOSTS[@]}"; do HOST_ARGS+=("$h"); done
    parallel -j 4 --halt now,fail=1 "$0" --single-host {} \
      $( [ "$FORCE_V4" = false ] && echo --ipv6 ) \
      $( [ "$DEBUG" -eq 1 ] && echo --debug ) \
      --format "$OUTPUT_FORMAT" \
      --services "$SERVICE_MAP" \
      $( ${CHECK_RELAY} && echo --check-relay ) \
      $( ${VERBOSE} && echo --verbose ) \
      $( ${INCLUDE_CLOUD} && echo --include-cloud ) \
      ::: "${HOST_ARGS[@]}"
    have_hosts=false
  fi
  if $have_hosts; then
...
run_tests() {
  ...
  if [[ ${#EXTRA_ENDPOINTS[@]} -gt 0 ]]; then
    for line in "${EXTRA_ENDPOINTS[@]}"; do
      ...
      if [[ "$eh" == "$h" ]]; then
        echo
        echo -e "  ${BOLD}Port $ep${NC} ${DIM}(${et^^})${NC}"
        test_one "$h" "$ep" "$et" || overall_rc=1
      fi
    done
  fi
...
exit $overall_rc  local host="$1"
  is_microsoft365_host "$host" && return 0
  is_google_workspace_host "$host" && return 0
  return 1
}
...
if [[ ${#POSITIONALS[@]} -gt 0 && ${#HOSTS[@]} -eq 0 && ${#DOMAINS[@]} -eq 0 ]]; then
  DOMAINS+=("${POSITIONALS[@]}"); DO_MX=true; fi

JSON_DATA=""

expand_services(){ local IFS=','; for p in $1; do local port="${p%%:*}" type="${p##*:}"; [ -z "$port" ]||[ -z "$type" ]&& die "Invalid service pair: '$p'"; echo "$port $type"; done; }
mx_hosts_for_domain(){ dig +short MX "$1" | sort -n -k1 | awk '{print $2}' | sed 's/\.$//' | awk 'NF'; }
fallback_hosts_for_domain(){ printf "%s\n" "mail.$1" "smtp.$1" "imap.$1" "$1"; }
has_a_record(){ dig +short A "$1" | grep -qE '^[0-9]+' && return 0; dig +short AAAA "$1" | grep -q ':' && return 0; return 1; }
get_dns_tlsa_rr(){ dig +short TLSA "_${2}._tcp.${1}" | head -1; }
join_txt() { sed -e 's/^"//' -e 's/"$//' -e 's/" "[ ]*//g'; }
...
dnssec_ad_flag(){
  ...
  local out hdr
  out="$(dig +dnssec "$type" "$name" $resolver 2>/dev/null || true)"
  hdr="$(printf "%s" "$out" | awk '/;; flags:/ {print $0}')"
  echo "$hdr" | grep -q ' ad[; ]' && echo "YES" || echo "NO"
}

dnssec_ds_present(){
  dig +short DS "$1" @1.1.1.1 | awk 'NF{print; exit}' >/dev/null && echo "YES" || echo "NO"
}

dnssec_rrsig_present(){
  dig "$1" "$2" +dnssec @1.1.1.1 | awk '/RRSIG/ {f=1} END{print (f?"YES":"NO")}'
}

check_reverse_dns() {
  local host="$1"
  local ip="$(dig +short A "$host" | head -1)"
  if [ -n "$ip" ]; then
    local rdns="$(dig +short -x "$ip" | sed 's/\.$//')"
    if [ -n "$rdns" ]; then
      echo "$rdns"
      return 0
    fi
...
check_caa_records() {
  local domain="$1"
  local caa="$(dig +short CAA "$domain" 2>/dev/null || true)"
  if [ -n "$caa" ]; then
    echo "$caa"
    return 0
  fi
...
check_bimi() {
  ...
  local name="${selector}._bimi.${domain}"
  local val; val="$(dig +short TXT "$name" 2>/dev/null | tr -d '\r' | join_txt || true)"
  if [[ -z "$val" ]]; then
    print_item "$INFO" "No BIMI record configured"
  else
    local v l a
    v="$(echo "$val" | sed -n 's/.*v=\([^;]*\).*/\1/p' || true)"
    l="$(echo "$val" | sed -n 's/.*l=\([^;]*\).*/\1/p' || true)"
    a="$(echo "$val" | sed -n 's/.*a=\([^;]*\).*/\1/p' || true)"
    if [[ "$v" == "BIMI1" ]]; then
      print_item "$OK" "BIMI configured${l:+ with logo}"
      ...
      print_item "$WARN" "Invalid BIMI version"
    fi
  fi
}
...
check_arc() {
  local domain="$1"
  local name="_domainkey.${domain}"
  local keys="$(dig +short TXT "$name" 2>/dev/null | grep -i "arc" || true)"
  if [ -n "$keys" ]; then
    print_item "$OK" "ARC authentication configured"
  else
    print_item "$INFO" "No ARC configuration (optional)"
  fi
}
...
analyze_ciphers() {
  ...
  if [ "$VERBOSE" = true ]; then
    ...
    local weak_ciphers="$(echo | openssl s_client -connect "${host}:${port}" $starttls -cipher "EXPORT:LOW:DES:RC4:MD5" 2>&1 | grep -i "cipher is" || true)"
    if [ -n "$weak_ciphers" ]; then
      print_detail "${RED}$WARN Weak ciphers supported!${NC}"
    fi

    local pref_cipher="$(echo | openssl s_client -connect "${host}:${port}" $starttls 2>&1 | grep "Cipher.*:" | head -1 | sed 's/.*Cipher.*: *//' || true)"
    if [ -n "$pref_cipher" ]; then
      log_verbose "Cipher: $pref_cipher"
    fi
  fi
}
...
spki_sha256() {
  ...
  hash=$(echo "$pem" \
    ...
    | eval "$HEXDUMP_CMD" | tr -d '\n' | tr '[:upper:]' '[:lower:]')
  [[ "$DEBUG" -eq 1 ]] && {
    local subj; subj="$("$OPENSSL" x509 -noout -subject 2>/dev/null <<<"$pem" | sed 's/^subject= //')"
    echo "SPKI subject: $subj" 1>&2; echo "SPKI sha256 : $hash" 1>&2; echo "---" 1>&2; }
  echo "$hash"
}
...
calculate_expected_tlsa() {
  ...
  if is_cloud_managed_host "$hostname"; then
    echo "PROVIDER_SKIP"
    return 0
  fi
...
check_mta_sts() {
  ...
  if [[ -z "$txt_val" ]]; then
    ...
    local v id
    v="$(kv_get_txt v "$txt_val")"
    id="$(kv_get_txt id "$txt_val")"
    if [[ "$(echo "$v" | tr '[:lower:]' '[:upper:]')" == "STSV1" ]]; then
      print_item "$OK" "MTA-STS configured"
      [ -n "$id" ] && print_detail "Version: ${BOLD}STSv1${NC}, ID: ${DIM}$id${NC}"
    else
      print_item "$WARN" "Invalid MTA-STS version in TXT"
    fi
  fi
  ...
  if $HAVE_CURL; then
    ...
    # Normalize, trim, and parse
    local pv pmode pmax
    pv="$(echo "$policy" | awk -F: 'BEGIN{IGNORECASE=1} /^[[:space:]]*version[[:space:]]*:/ {sub(/^[^:]*:[[:space:]]*/,""); gsub(/^[ \t]+|[ \t]+$/,""); print; exit}')"
    pmode="$(echo "$policy" | awk -F: 'BEGIN{IGNORECASE=1} /^[[:space:]]*mode[[:space:]]*:/    {sub(/^[^:]*:[[:space:]]*/,""); gsub(/^[ \t]+|[ \t]+$/,""); print; exit}')"
    pmax="$(echo "$policy" | awk -F: 'BEGIN{IGNORECASE=1} /^[[:space:]]*max_age[[:space:]]*:/ {sub(/^[^:]*:[[:space:]]*/,""); gsub(/^[ \t]+|[ \t]+$/,""); print; exit}')"
...
check_tls_rpt() {
  ...
  local val; val="$(check_txt_exists "$name" || true)"
  if [[ -z "$val" ]]; then
    print_item "$INFO" "No TLS-RPT configured"
  else
    local v rua; v="$(kv_get_txt v "$val")"; rua="$(kv_get_txt rua "$val")"
    local vnorm="$(echo "$v" | tr '[:lower:]' '[:upper:]')"
    if [[ "$vnorm" == "TLSRPTV1" || -z "$v" ]]; then
      print_item "$OK" "TLS-RPT configured"
      [ -n "$rua" ] && print_detail "Reports to: ${DIM}$rua${NC}"
    else
      print_item "$WARN" "Invalid TLS-RPT version"
    fi
  fi
}
...
check_spf() {
  ...
  local out; out="$(dig +short TXT "$domain" | tr -d '\r' | join_txt | grep -i 'v=spf1' || true)"
  if [[ -z "$out" ]]; then
    print_item "$WARN" "No SPF record"
  else
    print_item "$OK" "SPF configured"
    print_detail "${DIM}$out${NC}"
    if [ "$VERBOSE" = true ]; then
      if echo "$out" | grep -q "+all"; then
        log_verbose "${RED}Dangerous: +all allows anyone!${NC}"
      elif echo "$out" | grep -q "~all"; then
        log_verbose "Softfail mode (~all)"
      elif echo "$out" | grep -q "-all"; then
        log_verbose "${GREEN}Hardfail mode (-all)${NC}"
      fi
      local lookups=$(echo "$out" | grep -oE "(include:|a:|mx:|ptr:|exists:|redirect=)" | wc -l)
      if [ $lookups -gt 10 ]; then
        log_verbose "${YELLOW}$lookups DNS lookups (limit: 10)${NC}"
      fi
    fi
  fi
}
...
check_dmarc() {
  ...
  local val; val="$(check_txt_exists "$name" || true)"
  if [[ -z "$val" ]]; then
    print_item "$WARN" "No DMARC record"
  else
    local v p rua ruf fo sp adkim aspf pct
    v="$(kv_get_txt v "$val")"; p="$(kv_get_txt p "$val")"; sp="$(kv_get_txt sp "$val")"
    rua="$(kv_get_txt rua "$val")"; ruf="$(kv_get_txt ruf "$val")"
    fo="$(kv_get_txt fo "$val")"; adkim="$(kv_get_txt adkim "$val")"; aspf="$(kv_get_txt aspf "$val")"
    pct="$(kv_get_txt pct "$val")"
    local vnorm="$(echo "$v" | tr '[:lower:]' '[:upper:]')"
    if [[ "$vnorm" == "DMARC1" || -z "$v" ]]; then
      local policy_icon="$INFO"
      local policy_color=""
      case "${p:-none}" in
        none) policy_icon="$INFO"; policy_color="${YELLOW}";;
        quarantine) policy_icon="$WARN"; policy_color="${YELLOW}";;
      ...
      print_item "$WARN" "Invalid DMARC version"
    fi
  fi
}
...
check_dnssec_signals() {
  ...
  [ "$DEBUG" -eq 1 ] && echo "DEBUG: Checking DNSSEC for $domain" >&2
  ...
  local status="$INFO"
  local message="DNSSEC: "

  if [[ "$ds" == "YES" && "$rr" == "YES" && "$mxad" == "YES" ]]; then
    status="$OK"
    message="${message}${GREEN}Fully validated${NC}"
  elif [[ "$ds" == "YES" && "$rr" == "YES" && "$mxad" == "NO" ]]; then
    status="$WARN"
    message="${message}${YELLOW}Signed but validation failed${NC}"
  elif [[ "$ds" == "NO" && "$rr" == "NO" ]]; then
    status="$INFO"
    message="${message}Not signed"
  else
    status="$WARN"
    message="${message}${YELLOW}Partial/Invalid configuration${NC}"
  fi

  print_item "$status" "$message"
  if [ "$VERBOSE" = true ]; then
    log_verbose "DS at parent: $ds, RRSIG in zone: $rr, AD flag: $mxad"
    if [[ "$ds" == "YES" || "$rr" == "YES" ]]; then
      log_verbose "Validated using Cloudflare DNS (1.1.1.1)"
    fi
  fi
...
check_caa() {
  local domain="$1"
  local caa_records="$(check_caa_records "$domain" || true)"
  if [ -n "$caa_records" ]; then
    print_item "$OK" "CAA records configured"
    if [ "$VERBOSE" = true ]; then
    ...
    print_item "$INFO" "No CAA records"
  fi
}
...
output_csv() {
  echo "Host,Port,Service,Status,TLSA,TLS_Versions,Certificate_Days,DNSSEC"
}
...
[[ ${#DOMAINS[@]} -eq 0 && ${#HOSTS[@]} -eq 0 ]] && { usage; exit 1; }
...
if [[ ${#DOMAINS[@]} -gt 0 && $DO_MX == false ]]; then
  for d in "${DOMAINS[@]}"; do
    local_mx=(); readarray_compat local_mx mx_hosts_for_domain "$d"
    if [[ ${#local_mx[@]} -gt 0 ]]; then
      for mx in "${local_mx[@]}"; do
...
# Dedup hosts
[[ ${#HOSTS[@]} -gt 0 ]] && HOSTS=($(printf "%s\n" "${HOSTS[@]}" | sed 's/\.$//' | awk 'NF' | sort -u))

svc_pairs=(); readarray_compat svc_pairs expand_services "$SERVICE_MAP"
overall_rc=0
have_hosts=true
[[ ${#HOSTS[@]} -eq 0 ]] && have_hosts=false

# Only print Host Security Checks if we actually have hosts to probe
if [ "$OUTPUT_FORMAT" = "text" ] && $have_hosts; then
  print_section "Host Security Checks"
fi
...
if $have_hosts; then
  if [ "$PARALLEL" = true ] && command -v parallel >/dev/null 2>&1; then
    export -f test_one port_open run_s_client fetch_cert_chain calculate_expected_tlsa
    export -f spki_sha256 tls_versions_summary check_cert_expiry analyze_ciphers
    export -f check_smtp_banner check_open_relay check_reverse_dns
    export -f print_item print_detail print_subsection log_verbose
    export -f is_microsoft365_host is_google_workspace_host is_cloud_managed_host
    export FORCE_V4 DEBUG TIMEOUT_CMD OPENSSL HEXDUMP_CMD HAVE_NC HAVE_CURL
    export RED GREEN YELLOW CYAN NC BOLD BLUE MAGENTA DIM OK FAIL WARN INFO LOCK CERT
    export CHECK_RELAY VERBOSE INCLUDE_CLOUD
    parallel -j 4 run_tests ::: "${HOSTS[@]}" ::: "${svc_pairs[@]}"
  else
    for h in "${HOSTS[@]}"; do
      run_tests "$h" "${svc_pairs[@]}"
    done
  fi
fi

# Domain policy checks should run even if no hosts were tested
